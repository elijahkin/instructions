  ...
  Instruction *ChainRuleHelper(Instruction *instruction) {
    ...
    case kAcosh:
      return CreateBinary(
          ...
          CreateBinary(
              kSubtract,
              CreateBinary(kPower, instruction->operand(0), CreateConstant(2)),
              CreateConstant(1)),
          CreateConstant(-1 / 2.0));
    case kAsin:
      return CreateBinary(
          kPower,
          CreateBinary(
              kSubtract, CreateConstant(1),
              CreateBinary(kPower, instruction->operand(0), CreateConstant(2))),
          CreateConstant(-1 / 2.0));
    case kAsinh:
      return CreateBinary(
    ...
    case kAtanh:
      return CreateBinary(
          kDivide, CreateConstant(1),
          CreateBinary(kSubtract, CreateConstant(1),
                       CreateBinary(kPower, instruction->operand(0),
                                    CreateConstant(2))));
    case kCbrt:
      return CreateBinary(kMultiply, CreateConstant(1 / 3.0),
                          CreateBinary(kPower, instruction->operand(0),
                                       CreateConstant(-2 / 3.0)));
    case kCos:
    ...
    case kErf:
      return CreateBinary(
          kMultiply, CreateConstant(1.1283791671),
          CreateUnary(
              kExp,
    ...
    case kSqrt:
      return CreateBinary(kMultiply, CreateConstant(1 / 2.0),
                          CreateBinary(kPower, instruction->operand(0),
                                       CreateConstant(-1 / 2.0)));
    case kTan:
